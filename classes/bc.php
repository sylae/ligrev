...
class bc {
  ...
  function __construct($math) {
    ...
    $expr   = trim("scale=3; " . str_replace($remove, '',
        html_entity_decode($math)));
    ...
    if (php_uname('s') == "Windows NT") {
      ...
      $process = proc_open('timeout 5 bc -l ' . $this->libs, $descriptorspec,
        $pipes);
    }
    ...
    if (is_resource($process)) {
      ...
      $pipeout = trim(str_replace('\\' . PHP_EOL, '',
          stream_get_contents($pipes[1])));

      $stdout = (strlen($pipeout) > 80) ? substr($pipeout, 0, 77) . '...' : $pipeout;
      \Monolog\Registry::MATH()->debug("Received data from STDOUT",
        ['data' => $stdout]);

      $stderr = trim(str_replace('\\' . PHP_EOL, PHP_EOL,
          stream_get_contents($pipes[2])));
      \Monolog\Registry::MATH()->debug("Received data from STDERR",
        ['data' => $stderr]);
      ...
      \Monolog\Registry::MATH()->debug("Exited bc process",
        ['status_code' => $pinfo['exitcode']]);
      ...
      $this->result = sprintf(_("Ligrev error in bc parsing module: %s"),
        _("Process not created"));
    }
  }
