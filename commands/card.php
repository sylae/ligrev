...
class card extends \Ligrev\command {
  ...
  function process() {
    ...
    if (!array_key_exists($this->room, $decks)) {
      $this->_send($this->getDefaultResponse(), _("Deck uninitialized, use :shuffle."));
    } elseif (count($decks[$this->room]) == 0) {
      $this->_send($this->getDefaultResponse(), _("Deck depleted, use :shuffle."));
    } else {
      $c = array_pop($decks[$this->room]);
      $this->_send($this->getDefaultResponse(), sprintf(_("%s draws a %s"), $this->authorHTML, $c));
    }
  }
